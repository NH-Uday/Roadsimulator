...
# Define BUW colors
buw_colors <- list(
  primary = "#005ca9",     # BUW Dark Blue
  secondary = "#009ee3",   # BUW Light Blue
  accent = "#94c11c",      # BUW Green
...
ui <- dashboardPage(
  # Header with BUW colors and modern styling
  dashboardHeader(
    title = div(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      # Add BUW branding at bottom of sidebar
      div(
  ...
  # Body with modern card-based styling
  dashboardBody(
    ...
    tabItems(
      # Map View Tab with card-based design
      tabItem(tabName = "map_view",
              fluidRow(
                box(
                  width = 12,
                  title = "Network Section Map",
                  status = NULL,
                  solidHeader = FALSE,
                  leafletOutput("network_map", height = "600px"),
                  p("Click on a network section to select it for analysis.", 
                ...
                column(
                  ...
                  div(
                    ...
                    div(class = "box-header with-border", 
                        h3(class = "box-title", "Selected Section Info")),
                    div(class = "box-body", 
                        verbatimTextOutput("selected_section_info"))
                  )
                ),
                column(
                  width = 9,
                  div(
                    class = "box",
                    div(class = "box-header with-border",
                        h3(class = "box-title", "Selected Section Statistics")),
                    div(class = "box-body",
                        plotOutput("map_section_stats", height = "300px"))
                  )
                )
              )
      ),
              ...
              fluidRow(
                column(
                  ...
                  div(
                    class = "box",
                    div(class = "box-header with-border",
                        h3(class = "box-title", "Filter Options")),
                    div(class = "box-body",
                        ...
                        div(class = "filter-section",
                            ...
                            numericInput("network_section", "Section ID:",
                                         value = 99, min = 1, max = 850)
                        ),
                        ...
                        div(class = "filter-section",
                            ...
                            checkboxGroupInput("traffic_times", "Traffic Time Periods:",
                                               ...
                                               selected = c("morgendliche Hauptverkehrszeit", 
                                                            "morgendliche Nebenverkehrszeit", 
                                                            "abendliche Hauptverkehrszeit", 
                                                            "abendliche Nebenverkehrszeit"))
                        ),
                ...
                column(
                  ...
                  div(
                    class = "box",
                    div(class = "box-header with-border",
                        h3(class = "box-title", "Travel Time Analysis Plot")),
                    div(class = "box-body",
      ...
      tabItem(tabName = "explanations",
              div(
                class = "box",
                div(class = "box-header with-border",
                    h3(class = "box-title", "About this Analysis")),
                div(class = "box-body",
                    h3("Travel Time Analysis for German Highway Sections", 
                       style = paste0("color: ", buw_colors$primary, "; font-weight: 600;")),
                    p("This application visualizes travel time data for trucks on various freeway facilities in Germany. 
                       The travel times are derived from Floating Car Data for the year 2019.",
                      style = "margin-bottom: 20px;"),
                    
                    div(
                      style = "background: #f5f8fb; border-left: 4px solid #005ca9; padding: 15px; margin-bottom: 20px;",
                      h4("Key Metrics:", style = "margin-top: 0;"),
                      tags$ul(
                        style = "margin-bottom: 0;",
                        tags$li(strong("Mean Travel Time:"), "The average travel time in Minutes"),
                        tags$li(strong("Coefficient of Variation:"), "A statistical measure showing the dispersion of travel times. Higher values indicate less predictable travel times."),
                        tags$li(strong("Buffer Time Index:"), "A measure of travel time reliability. It indicates how much extra time travelers should add to their trips to ensure on-time arrival with 95% certainty."),
                        tags$li(strong("Percentiles of travel times:"), "Different percentiles of travel times to depict the skewness of travel times.")
                      )
                    ),
                    
                    div(
                      style = "background: #f8fbf5; border-left: 4px solid #94c11c; padding: 15px; margin-bottom: 20px;",
                      h4("Traffic Time Periods:", style = "margin-top: 0;"),
                      tags$ul(
                        style = "margin-bottom: 0;",
                        tags$li(strong("Morning Rush Hour:"), "Peak traffic period in the morning between 6:00-8:00."),
                        tags$li(strong("Morning Off-Peak:"), "Non-peak traffic period in the morning between 4:00-6:00."),
                        tags$li(strong("Evening Rush Hour:"), "Peak traffic period in the evening between 16:00-18:00."),
                        tags$li(strong("Evening Off-Peak:"), "Non-peak traffic period in the evening between 18:00-20:00")
                      )
                    )
                )
              )
      ...
      tabItem(tabName = "acknowledgement",
              div(
                class = "box",
                div(class = "box-header with-border",
                    h3(class = "box-title", "Acknowledgements")),
                div(class = "box-body",
                    div(
                    ...
                    div(
                      style = "text-align: center; margin-top: 20px; background: #f5f8fb; padding: 20px; border-radius: 6px;",
...
server <- function(input, output, session) {
  ...
  output$network_map <- renderLeaflet({
    ...
    print("Checking avg_velocity column...")
    print(names(network_geodata()))  
    print(summary(network_geodata()$velocity_65))  
    
    if (!("avg_velocity" %in% names(network_geodata()))) {
      print("FEHLER: avg_velocity existiert nicht!")
    }
    
    if (any(is.na(network_geodata()$velocity_65))) {
      print("WARNUNG: Es gibt NA-Werte in avg_velocity!")
    }
    
    if (!is.numeric(network_geodata()$velocity_65)) {
      print("FEHLER: avg_velocity ist kein numerischer Wert!")
      network_geodata() <- network_geodata() %>%
        mutate(velocity_65 = as.numeric(velocity_65))
    }
    
    pal <- colorNumeric(
      palette = "viridis",
      domain = network_geodata()$velocity_65
    )
    
    leaflet(network_geodata()) %>%
      #addTiles() %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolylines(  # 🚀 Statt addCircleMarkers() jetzt addPolylines()
        color = ~pal(velocity_65),
        weight = 4,  # Liniendicke
        opacity = 0.8,
        popup = ~paste("<strong>Netzabschnitt:</strong>", netzabschnitt, "<br>",
                       #"<strong>Name:</strong>", name, "<br>",
                       "<strong>35% schnellste Geschwindigkeit in der Nebenverkehrszeit:</strong>", round(velocity_65, 3), "km/h"),
        layerId = ~netzabschnitt
      ) %>%
      addLegend(
        position = "bottomright",
        pal = pal,
        values = network_geodata()$velocity_65,
        title = "35% schnellste Geschwindigkeit (km/h)",
        opacity = 0.8
      )
  })
  ...
  observeEvent(input$network_map_marker_click, {
    click_id <- input$network_map_marker_click$id
    if (!is.null(click_id)) {
      selected_section(click_id)
      # Update the numericInput field
      updateNumericInput(session, "network_section", value = click_id)
      # Switch to visualizations tab
      updateTabItems(session, "sidebarMenu", "visualizations")
    }
  })
